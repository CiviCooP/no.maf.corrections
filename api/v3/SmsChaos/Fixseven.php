 ...
 */
function civicrm_api3_sms_chaos_fixseven($params) {
  set_time_limit(0);
  $returnValues = array();
  $smsTagId7 = civicrm_api3('Tag', 'getvalue', array('name' => 'SMS Chaos 7', 'return' => 'id'));
  ...
  $query = 'SELECT DISTINCT(entity_id) FROM civicrm_entity_tag WHERE entity_table = %1 AND tag_id = %2';
  $queryParams = array(
    1 => array('civicrm_contact', 'String'),
    2 => array($smsTagId7, 'Integer'));
  ...
  return civicrm_api3_create_success($returnValues, $params, 'SMSChaos', 'Fixseven');
}
